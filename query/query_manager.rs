...
impl QueryManager {
    ...
    pub fn prepare_read_pipeline<Snapshot: ReadableSnapshot + 'static>(
        ...
        let arced_inputs = translated_inputs.map(Arc::new);
    ...
    pub fn analyse<Snapshot: ReadableSnapshot + 'static>(
        ...
        let annotated_pipeline = annotate_preamble_and_pipeline(
            ...
            &parameters,
            (*arced_preamble).clone(),
            arced_inputs.map(|inputs| (*inputs).clone()),
            (*arced_stages).clone(),
            (*arced_fetch).clone(),
        )
...
fn annotate_and_compile_query(
    ...
    arced_inputs: Option<Arc<TranslatedInputs>>,
    ...
    let AnnotatedPipeline { annotated_preamble, annotated_inputs, annotated_stages, annotated_fetch } =
        annotated_pipeline;
