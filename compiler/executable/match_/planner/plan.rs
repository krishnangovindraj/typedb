    ...
    for pattern in conjunction.nested_patterns() {
        match pattern {
            NestedPattern::Disjunction(disjunction) => {
                let planner = DisjunctionPlanBuilder::new(disjunction.conjunctions_by_branch_id().map(|(id, _)| *id).collect(),
                    disjunction
                        .conjunctions()
                        .iter()
                        .map(|branch| {
            ...
            NestedPattern::Optional(optional) => {
                let parent_bound_variables: HashSet<_> = optional.required_inputs(block_context).collect();
                let optional_vars = optional.optional_variables(block_context).collect();
...
impl PartialCostPlan {
    ...
    fn add_to_stash(&mut self, pattern: PatternVertexId, graph: &Graph<'_>) {
        ...
        self.ongoing_step_stash_produced_vars
            .extend(graph.elements[&VertexId::Pattern(pattern)].variables());
    }
...
impl ConjunctionPlan<'_> {
            ...
            match &self.graph.elements()[&VertexId::Pattern(producer)] {
                ...
                PlannerVertex::Disjunction(disjunction) => {
                    ...
                    .lower(
                        ...
                        conjunction_builder.current_outputs.iter().copied().collect(),
    ...
    fn may_make_check_step(
        ...
        match &self.graph.elements()[&VertexId::Pattern(pattern)] {
            ...
            PlannerVertex::Disjunction(disjunction) => {
                let step_builder = disjunction.builder().clone().plan(conjunction_builder.position_mapping().keys().copied())?.lower(
                    self.local_annotations.vertex_annotations(),
                    conjunction_builder.row_variables().iter().copied(),
                    conjunction_builder.current_outputs.clone(),
                    conjunction_builder.position_mapping(),
...
impl<'a> Graph<'a> {
    ...
    fn push_optional(&mut self, optional: OptionalVertex<'a>) {
        ...
        self.pattern_to_variable
            .entry(pattern_index)
            .or_default()
            .extend(optional.variables());
        for var_id in optional.variables() {
