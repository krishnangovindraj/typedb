...
use crate::{
    pattern::{
        conjunction::{Conjunction, ConjunctionBuilder, ConjunctionBuilderWithContext}, impl_pattern_from_pattern_variables, nested_pattern::NestedPattern, BranchID, ContextualisedBindingMode, Pattern,
        PatternVariables,
        Scope,
        ScopeId,
    },
...
use crate::pattern::mode_inference::{VariableBindingMode, VariableUsageMode};
...
impl DisjunctionBuilder {
    ...
    pub(crate) fn variable_usage_modes(&self) -> HashMap<Variable, VariableUsageMode> {
        ...
        binding_modes
            .iter_mut()
            .filter(|(_, mode)| mode.binding_mode.is_locally_binding_in_child())
            .for_each(|(var, mode)| {
                ...
                if binding_branches_count > 1 {
                    mode.binding_mode = VariableBindingMode::RequirePrebound
                }
            });
        binding_modes
    }
