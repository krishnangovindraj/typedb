...
pub fn infer_types(
    ...
    // let mut function_annotations = Vec::with_capacity(program.functions().len());
    // TODO: Extend to functions when we implement them
    // for function in program.functions() {
    //     // TODO: We don't use the full set of annotations for these functions, so we don't always detect unsatisfiable queries
    //     function_annotations.push(infer_types_for_function(function, snapshot, type_manager, schema_functions));
    // }
    let (entry, functions) = program.into_parts();
    let local_function_cache = infer_types_for_functions(functions, snapshot, type_manager, &schema_functions)?; // todo!();
    let root_tig =
        infer_types_for_block(snapshot, &entry, type_manager, &schema_functions, Some(&local_function_cache))?;
    let entry_annotations = TypeAnnotations::build(root_tig);
    Ok(AnnotatedProgram::new(entry, entry_annotations, local_function_cache, schema_functions.clone()))
}
...
pub fn infer_types_for_functions(
    ...
    // TODO: Do we want to return Vec<Result> instead?
...
pub fn infer_types_for_function(
    ...
    Ok(FunctionAnnotations { return_annotations, annotations: body_annotations })
}
...
pub struct FunctionAnnotations {
    pub(crate) annotations: TypeAnnotations,
...
pub mod tests {
    ...
    use crate::{
        ...
        program::{
            ...
            function::{FunctionID, FunctionIR, ReturnOperationIR},
    ...
    fn test_functions() {
        ...
        .map(|function_id| {
            ...
            let f_ir = FunctionIR::new(builder.finish(), [].into_iter(), ReturnOperationIR::Stream(vec![f_var_animal]))
                .unwrap();
            ...
            let call = {
                let returns = f_ir
                    ...
                    .iter()
                    .map(|v| {
                        let context = f_ir.block().context();
                        (
                            context.get_variable_category(f_var_animal).unwrap(),
                            if context.is_variable_optional(f_var_animal) {
                ...
                FunctionCall::new(
                    ...
                    HashMap::new(),
                    returns,
                    f_ir.return_operation().is_stream(),
                )
            };
            conjunction.constraints_mut().add_function_call(vec![var_animal], call.clone()).unwrap();
        ...
        let f_annotations = {
            ...
            let isa = f_ir.block().conjunction().constraints().constraints()[0].clone();
            ...
            assert_eq!(
                f_annotations.annotations.variables,
                HashMap::from([
