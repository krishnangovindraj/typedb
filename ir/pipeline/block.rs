...
fn validate_no_optionals_in_negations(
    ...
    if this_conjunction_in_negation {
        if let Some(optional) = conjunction.nested_patterns().iter().filter_map(|nested| nested.as_optional()).next() {
    ...
    conjunction.nested_patterns().iter().try_for_each(|nested| match nested {
        ...
        NestedPattern::Negation(negation) => validate_no_optionals_in_negations(negation.conjunction(), true),
...
impl<'a> BlockBuilderContext<'a> {
    ...
    pub(crate) fn input_variables(&self) -> impl Iterator<Item = Variable> + '_ {
        self.block_context.variable_declaration.iter().copied().filter(|var| self.is_variable_input(*var))
    }
