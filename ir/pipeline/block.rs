...
use crate::{
    RepresentationError,
    pattern::{
        BranchID, Pattern, PatternVariables, ScopeId,
        conjunction::{Conjunction, ConjunctionBuilder, ConjunctionBuilderWithContext, NestedPatternBuilder},
        constraint::Constraint,
        mode_inference::{AssignmentStatus, BindingMode, OptionalReferenceSafety, VariableUsageMode},
...
impl<'reg> BlockBuilder<'reg> {
    ...
    pub fn finish(mut self) -> Result<Block, Box<RepresentationError>> {
        ...
        self.context.variable_names_index.retain(|_, var| {
            block_binding_modes.get(var).map(|mode| mode.binding_mode) != Some(BindingMode::LocallyBindingInChild)
        });
        let conjunction =
    ...
    fn variable_usage_modes(&self) -> HashMap<Variable, VariableUsageMode> {
        ...
        for (id, optionality) in self.context.input_variable_optionalities() {
            ...
            mode.binding_mode = BindingMode::AlwaysBinding;
...
fn validate_optional_returns_recursive(
    ...
    let reused_optional_return_opt = acc.iter().find(|(var, _)| match conjunction_binding_modes.get(var) {
        None => false,
        Some(mode) => mode.binding_mode != BindingMode::BoundInTry,
    });
...
fn validate_expressions_assignments_are_unique(
    ...
    for (id, _) in context.input_variable_optionalities() {
        let assignment_mode =
            variable_usage_modes.get(&id).map_or(AssignmentStatus::NotAssigned, |mode| mode.assignment);
        ...
        let enabled_constraint_indices = remaining_constraint_indices
            ...
            .filter(|i| {
                let mut required_vars = constraint_at(*i)
                    .variable_usage_modes()
                    .filter_map(|(id, mode)| mode.binding_mode.is_require_prebound().then_some(id));
...
impl UnplannableConstraints {
    fn build<'conj>(
        ...
        constraints_and_requirements.extend(remaining_constraints.map(|c| {
            let required_vars =
                c.variable_usage_modes().filter_map(|(id, mode)| mode.binding_mode.is_require_prebound().then_some(id));
